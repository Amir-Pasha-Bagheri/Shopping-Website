...
class Food extends Component {
    render(){
        const cheap = () =>{
            const a = this.props.status.food
        ...
        const add = (a) =>{
            const ThisID = this.props.status.food.map(function(e){return e.name}).indexOf(a.name)
            this.props.dispatch({type:act.ADD, kind:"food", product:ThisID})
            if(a.amount>=0){
                this.props.dispatch({type:act.DOEXIST,CartID:a.CartID,name:a.name,amount:a.amount,price:a.price,img:a.img,id:ThisID,kind:"food"})
        ...
        const remove = (a) =>{
            const ThisID = this.props.status.food.map(function(e){return e.name}).indexOf(a.name)
            if(a.amount>1){
                this.props.dispatch({type:act.REM, kind:"food", product:ThisID})
            ...
            <React.Fragment>
                <Navbar bg="dark" variant="dark" style={{fontSize:13}}>
                    ...
                    <Nav className="ml-2 mr-auto">
                        ...
                        <Nav.Link className="ml-2"><Link style={{textDecoration:"none"}} className="nav-link active">Foods</Link></Nav.Link>
                ...
                <Container>
                    ...
                    {this.props.status.food.map((a,index)=><Stuff key={index} name={a.name} amount={a.amount} price={a.price} img={a.img} add={()=>add(a)} remove={()=>remove(a)}/>)}
                </Container>
            </React.Fragment>
        )
    }
