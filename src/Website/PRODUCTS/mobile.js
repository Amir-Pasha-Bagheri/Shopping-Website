...
class Mobile extends Component {
    render(){
        const add = (a) =>{
            this.props.dispatch({type:act.ADD, kind:"mobile", product:a.id})
            this.props.dispatch({type:act.DOEXIST,CartID:a.CartID,name:a.name,amount:a.amount,price:a.price,img:a.img,id:a.id,kind:"mobile"})
        ...
        const remove = (a) =>{
        if(a.amount>1){
            this.props.dispatch({type:act.REM, kind:"mobile", product:a.id})
        ...
        return(
            <React.Fragment id="mobile-item">
                ...
                <Container>
                    ...
                    {this.props.status.mobile.map((e,index)=><Stuff key={index} name={e.name} amount={e.amount} price={e.price} cart={this.props.status.existOfItems} img={e.img} add={()=>add(e)} remove={()=>remove(e)}/>)}
                </Container>
            </React.Fragment>
        )
    }
