...
class Clothes extends Component {
    render(){
        const cheap = () =>{
            const a = this.props.status.clothes
        ...
        const add = (a) =>{
            const ThisID = this.props.status.clothes.map(function(e){return e.name}).indexOf(a.name)
            this.props.dispatch({type:act.ADD, kind:"clothes", product:ThisID})
            if(a.amount>=0){
                this.props.dispatch({type:act.DOEXIST,CartID:a.CartID,name:a.name,amount:a.amount,price:a.price,img:a.img,id:ThisID,kind:"clothes"})
        ...
        const remove = (a) =>{
            const ThisID = this.props.status.clothes.map(function(e){return e.name}).indexOf(a.name)
            if(a.amount>1){
                this.props.dispatch({type:act.REM, kind:"clothes", product:ThisID})
        ...
        return(
            <React.Fragment>
                ...
                <Container>
                    ...
                    {this.props.status.clothes.map((a,index)=><Stuff key={index} name={a.name} amount={a.amount} price={a.price} img={a.img} add={()=>add(a)} remove={()=>remove(a)}/>)}
                </Container>
            </React.Fragment>
        )
    }
