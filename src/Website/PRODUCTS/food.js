...
class Food extends Component {
    render(){
        const add = (a) =>{
            this.props.dispatch({type:act.ADD, kind:"food", product:a.id})
            this.props.dispatch({type:act.DOEXIST,CartID:a.CartID,name:a.name,amount:a.amount,price:a.price,img:a.img,id:a.id,kind:"food"})
        ...
        const remove = (a) =>{
        if(a.amount>1){
            this.props.dispatch({type:act.REM, kind:"food", product:a.id})
        ...
        return(
            <React.Fragment>
                ...
                <Container>
                    <h2 className="my-4" id="food-item">Foods :</h2>
                    {this.props.status.food.map((e,index)=><Stuff key={index} name={e.name} amount={e.amount} price={e.price} cart={this.props.status.existOfItems} img={e.img} add={()=>add(e)} remove={()=>remove(e)}/>)}
                </Container>
            </React.Fragment>
        )
    }
